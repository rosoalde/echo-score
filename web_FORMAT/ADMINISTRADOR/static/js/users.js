...
async function openView(id) {
    ...
    if (!res.ok) {
        alert(u.error || "No se pudo cargar el usuario");
        return;
    }
    ...
    document.getElementById("viewIdBadge").textContent = `ID #${u.id}`;

    const statusBadge = document.getElementById("viewStatusBadge");
    statusBadge.textContent = u.is_active ? "Activo" : "Inactivo";
    statusBadge.className = `badge ${u.is_active ? "bg-success" : "bg-danger"}`;

    const verifiedBadge = document.getElementById("viewVerifiedBadge");
    verifiedBadge.textContent = u.is_verified ? "Verificado" : "Sin verificar";
    verifiedBadge.className = `badge ${u.is_verified ? "bg-primary" : "bg-warning text-dark"}`;
    ...
    if (u.roles && u.roles.length) {
        ...
        rolesEl.innerHTML = '<span class="text-muted">Sin roles asignados</span>';
    }

    document.getElementById("viewSessionsCount").textContent = u.sessions_count ?? 0;
    document.getElementById("viewLastIp").textContent = u.last_session?.ip_address || "—";
    document.getElementById("viewLastSession").textContent = u.last_session
        ? new Date(u.last_session.created_at).toLocaleString()
        : "Sin conexiones registradas";

    document.getElementById("viewTasksCount").textContent = u.tasks_count ?? 0;

    // proyectos / análisis
    const analyses = u.analyses || { total: 0, by_status: {}, items: [] };

    document.getElementById("viewAnalysesTotal").textContent = analyses.total;

    const byStatusEl = document.getElementById("viewAnalysesByStatus");
    byStatusEl.innerHTML = "";
    Object.entries(analyses.by_status || {}).forEach(([status, count]) => {
        const badge = document.createElement("span");
        badge.className = `badge ${STATUS_STYLES_ANALYSIS[status] || "bg-secondary"}`;
        badge.textContent = `${status}: ${count}`;
        byStatusEl.appendChild(badge);
    });

    const tbody = document.getElementById("viewAnalysesBody");
    tbody.innerHTML = "";

    if (!analyses.items || !analyses.items.length) {
        tbody.innerHTML = `
            <tr><td colspan="4" class="text-muted text-center py-3">
                Este usuario no tiene proyectos todavía.
            </td></tr>`;
    } else {
        analyses.items.forEach(a => {
            const tr = document.createElement("tr");
            tr.innerHTML = `
                <td>${a.project_name}</td>
                <td><span class="badge ${STATUS_STYLES_ANALYSIS[a.status] || "bg-secondary"}">${a.status}</span></td>
                <td>${a.progress_percent || 0}%</td>
    ...
    document.getElementById("viewAnalysesMore").style.display = analyses.total > 20 ? "block" : "none";

    // botón "editar este usuario" dentro del modal de vista
