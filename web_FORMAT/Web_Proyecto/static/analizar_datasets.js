document.addEventListener("DOMContentLoaded", () => {
    ...
    const PLATFORM_ORDER = ["bluesky", "reddit", "telegram", "youtube"];
    function sortByPlatformOrder(arr, getName) {
        getName = getName || function (x) { return x; };
        return arr.slice().sort(function (a, b) {
            var ia = PLATFORM_ORDER.indexOf((getName(a) || "").toLowerCase());
            var ib = PLATFORM_ORDER.indexOf((getName(b) || "").toLowerCase());
            if (ia === -1) ia = 999;
            if (ib === -1) ib = 999;
            return ia - ib;
        });
    ...
    function extractPlatformData(porPlataforma) {
        ...
        platforms = sortByPlatformOrder(platforms);
    ...
    async function cargarDashboard(analysisId, retryCount) {
        ...
        try {
            ...
            // Aunque el LLM esté caído (botón "Indicador de aceptación" deshabilitado),
            // si ya existe un cálculo previo lo mostramos igualmente (solo lectura).
            cargarAceptacionExistente(analysisId);
    ...
    function renderDashboard(data) {
        ...
        try {
            ...
            if (scoreop.disponible && scoreop.por_plataforma) {
                ...
                const volKeysSorted = sortByPlatformOrder(Object.keys(vol));
                volLabels = volKeysSorted;
                volValues = volKeysSorted.map(function (k) { return vol[k]; });
            }
        ...
        try {
            ...
            const redes = sortByPlatformOrder(Object.keys(trendRed).filter(red =>
    ...
    function renderScoreOP(scoreop) {
        ...
        var platsConDatos = scoreop.tendencia_red ? sortByPlatformOrder(Object.keys(scoreop.tendencia_red)) : [];
        ...
        <div class="bg-light p-3 rounded-3 mb-4 border-start border-4 border-success shadow-sm">
            ...
            <p class="text-muted mb-0" style="font-size:.75rem;">
                ...
                La etiqueta <em>Agenda X</em> identifica las publicaciones con mayor presencia e influencia en la conversación.
            </p>
        </div>
        <div class="row g-4">
            <div class="col-md-6">
    ...
    // ════════════════════════════════════════════════════════
    // 9a-bis. LEER (SIN CALCULAR) UN INDICADOR DE ACEPTACIÓN YA EXISTENTE
    // Se usa al cargar el dashboard, independientemente de si el LLM
    // está disponible o no: solo lee el JSON generado previamente.
    // Si nunca se calculó (respuesta vacía), no hace nada y el botón
    // sigue con el estado que le imponga la lógica de disponibilidad de LLM.
    // ════════════════════════════════════════════════════════
    async function cargarAceptacionExistente(analysisId) {
        if (!analysisId) return;
        try {
            var response = await fetch("/analisis/" + analysisId + "/read_aceptacion", {
                method: "POST",
                headers: { "Content-Type": "application/json" }
            });
            if (!response.ok) return; // sin permisos, análisis no encontrado, etc. -> no mostramos nada

            var data = await response.json();

            // read_indicador_aceptacion devuelve {} cuando no existe aceptacion_global.json;
            // en ese caso el endpoint responde {"ok": true} sin la clave "global".
            var hayResultadoPrevio = data && data.global && Object.keys(data.global).length > 0;
            if (!hayResultadoPrevio) return;

            renderAceptacion(data, /* openModal = */ false);
        } catch (err) {
            console.warn("No se pudo comprobar si existe un indicador de aceptación previo:", err.message);
        }
    }
    ...
    // ════════════════════════════════════════════════════════
    function renderAceptacion(result, openModal) {
        if (openModal === undefined) openModal = true; // comportamiento previo: botón y filtro-geo siguen abriendo el modal
        ...
        var redes = sortByPlatformOrder(Object.keys(porRed));
        ...
        if (bodyEl) {
            bodyEl.innerHTML =
                ...
                `<div class="alert alert-info border-0 small mb-3 py-2 px-3" style="background:#e8f4fd;">
                    ...
                    <p class="mb-1">
                        La IA analiza el contenido de las publicaciones y los comentarios para identificar cómo se manifiestan los cuatro pilares de aceptación: legitimación, efectividad percibida, justicia y equidad, y confianza institucional. A partir de este análisis, la plataforma organiza los resultados por pilar y red social.
                    </p>
                    <p class="mb-0">
                        La tabla muestra el resultado de cada pilar en cada plataforma. La columna «Resultado global» reúne el promedio de todas las redes para cada dimensión, mientras que la fila «Aceptación total» resume la valoración conjunta de los cuatro pilares en cada plataforma.
                    </p>
                </div>` +
        ...
        // ── Abrir modal (solo si se pidió explícitamente) ─────────────────
        if (openModal) {
